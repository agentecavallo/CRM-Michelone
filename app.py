...
# Funzione "magica" per evitare l'errore rosso (StreamlitAPIException)
def applica_dati_gps():
    if 'gps_temp' in st.session_state:
        ...
        # Cancelliamo i dati temporanei per nascondere il box di conferma
...
with st.expander("➕ REGISTRA NUOVA VISITA", expanded=True):
    ...
    # Layout Campi: [3 parti Località, 1 parte Provincia]
    # Nota: Su mobile si impilano automaticamente per essere leggibili.
    ...
    if loc_data:
        ...
        if st.button("📍 CERCA POSIZIONE GPS", use_container_width=True):
            try:
                ...
                # Salviamo in 'gps_temp' (senza toccare ancora i campi visibili)
                ...
                st.warning("Segnale GPS OK, ma indirizzo sconosciuto.")

        # 2. Conferma (appare solo se abbiamo trovato qualcosa)
        if 'gps_temp' in st.session_state:
            dati = st.session_state['gps_temp']
            ...
            with c_yes:
                # QUI STA LA MAGIA: on_click=applica_dati_gps
                # Questo esegue l'aggiornamento PRIMA di ridisegnare la pagina, evitando l'errore rosso.
                st.button("✅ INSERISCI", on_click=applica_dati_gps, use_container_width=True)
            with c_no:
