...
with st.expander(titolo_form, expanded=st.session_state.editing_id is not None):
    # Recupero dati se in modalità editing
    val_cliente = ""
    val_note = ""
    val_loc = ""
    val_prov = ""
    ...
    if st.session_state.editing_id:
        ...
        conn.close()
        if curr:
            val_cliente = curr[1]; val_loc = curr[2]; val_prov = curr[3]
            val_note = curr[6]; val_agente = curr[9]
    ...
    with c2: c_agente = st.selectbox("Agente", ["HSE", "BIENNE", "PALAGI", "SARDEGNA"], index=["HSE", "BIENNE", "PALAGI", "SARDEGNA"].index(val_agente) if val_agente in ["HSE", "BIENNE", "PALAGI", "SARDEGNA"] else 0)
    ...
    with col_save:
        if st.button("💾 SALVA", use_container_width=True):
            ...
            if c_fup in mappa:
                data_fup = (c_data + timedelta(days=mappa[c_fup])).strftime("%Y-%m-%d")
            
            dati_tuple = (c_nome, c_loc.upper(), c_prov.upper(), c_tipo, c_data.strftime("%d/%m/%Y"), c_note, data_fup, c_data.strftime("%Y-%m-%d"), c_agente)
            
            if st.session_state.editing_id:
                salva_visita_db(dati_tuple, st.session_state.editing_id)
                st.session_state.editing_id = None
            else:
                salva_visita_db((*dati_tuple, "", ""), None) # Lat/Lon vuote per inserimento manuale
            
            st.toast("Operazione completata!")
            st.rerun()
...
# --- 5. ARCHIVIO CON MODIFICA ED ELIMINA ---
...
if t_ricerca or st.button("MOSTRA TUTTE"):
    ...
    for _, row in df.iterrows():
        with st.expander(f"{row['data']} - {row['cliente']} ({row['agente']})"):
            ...
            c_mod, c_del, c_map = st.columns([1, 1, 2])
            
            # Tasto MODIFICA
            if c_mod.button("✏️ Modifica", key=f"edit_{row['id']}"):
                st.session_state.editing_id = row['id']
                st.rerun()
            
            # Tasto ELIMINA con conferma
            if c_del.button("🗑️ Elimina", key=f"del_{row['id']}"):
                elimina_visita_db(row['id'])
                st.toast("Visita eliminata")
                st.rerun()
                
            if row['latitudine']:
                c_map.markdown(f"[📍 Mappa](http://maps.google.com/?q={row['latitudine']},{row['longitudine']})")
