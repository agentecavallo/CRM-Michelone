        ...
        for _, row in df_new.iterrows():
            c.execute("""INSERT INTO visite (cliente, localita, provincia, tipo_cliente, data, note, 
                      ...
                      (row['cliente'], row['localita'], row['provincia'], row['tipo_cliente'], 
                       row['data'], row['note'], row['data_followup'], row['data_ordine'], 
                       row['agente'], row['latitudine'], row['longitudine']))
...
with st.expander("➕ REGISTRA NUOVA VISITA", expanded=False):
    ...
    st.button("💾 SALVA VISITA", on_click=salva_vis_ita, use_container_width=True)
    ...
    if not df.empty:
        for _, row in df.iterrows():
            with st.expander(f"{row['data']} - {row['cliente']} ({row['agente']})"):
                ...
                if row['latitudine']:
                    st.markdown(f"[📍 Vai su Mappa](https://www.google.com/maps?q={row['latitudine']},{row['longitudine']})")
    else:
...
with st.expander("💾 BACKUP E RIPRISTINO"):
    if st.download_button("📦 SCARICA BACKUP EXCEL", genera_excel_backup(), f"Backup_CRM_{oggi}.xlsx", use_container_width=True):
        st.success("Backup generato!")
    
    st.markdown("---")
    file_restore = st.file_uploader("Ripristina da Backup", type=["xlsx"])
    if file_restore and st.button("🚨 SOVRASCRIVI DATI", use_container_width=True):
