...
if not df_scadenze.empty:
    ...
    for _, row in df_scadenze.iterrows():
        ...
        with st.container(border=True):
            ...
            st.caption(f"📅 {msg_scadenza}")
            # Visualizzazione note con tasto copia nativo anche negli alert
        ...
        for _, row in df.iterrows():
            ...
            with st.expander(f"{icona_crm} {row['data']} - {row['cliente']} {badge_tipo}"):
                if st.session_state.edit_mode_id == row['id']:
                    ...
                    new_fup = datetime.now().strftime("%Y-%m-%d") if attiva_fup else ""
                    if attiva_fup:
                        new_fup = st.date_input("Nuova Data Ricontatto", value=val_ini, key=f"e_dt_{row['id']}").strftime("%Y-%m-%d")
                    ...
                    # --- MODALITÀ VISUALIZZAZIONE COMPLETA ---
                    ...
                    # SISTEMA DI COPIA NATIVO
                    st.code(row['note'], language=None)
                    
                    is_copied = True if row.get('copiato_crm') == 1 else False
                    if st.checkbox("✅ Salvato su CRM", value=is_copied, key=f"chk_crm_{row['id']}"):
                        if not is_copied:
                            with sqlite3.connect('crm_mobile.db') as conn:
                    ...
                    if cb_m.button("✏️ Modifica", key=f"btn_mod_{row['id']}", use_container_width=True):
                        st.session_state.edit_mode_id = row['id']
                        st.rerun()
                    
                    key_conf = f"confirm_del_{row['id']}"
                    if cb_d.button("🗑️ Elimina", key=f"btn_del_{row['id']}", use_container_width=True):
...
with st.expander("🛠️ AMMINISTRAZIONE E BACKUP"):
    ...
    if os.path.exists(cartella_backup):
        ...
        else: st.info("Nessun backup automatico generato finora.")
...
with col_f2:
    try:
        ...
        st.markdown("<p style='text-align: center; font-weight: bold;'>✅ MICHELONE APPROVED</p>", unsafe_allow_html=True)
